...
/*   Updated: 2025/09/25 11:15:45 by cgross-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int pipe_fd[2], pid_t *last_pid, char **envp)
{
	...
	if (!create_pipe_and_fork(cmd, pipe_fd, &pid))
		return (0);
	if (pid == 0)
		handle_child_process(cmd, *input_fd, pipe_fd, envp);
	if (pid > 0)
		*last_pid = pid;
	update_file_descriptors(input_fd, pipe_fd, cmd);
	return (1);
}
...
static void	update_file_descriptors(int *input_fd, int pipe_fd[2], t_command *cmd)
{
	if (*input_fd != STDIN_FILENO)
		close(*input_fd);
	if (cmd->next)
