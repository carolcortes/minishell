...
/*   Updated: 2025/09/22 16:40:12 by cgross-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int pipe_fd[2], char **envp)
{
	...
	// 2. Redirecionamentos de ARQUIVO
	printf("Applying %d file redirections\n", cmd->redir_count);
	if (!apply_redirections(cmd))
		exit(1);
	...
	int pipe_fd[2], char **envp)
{
	...
	printf("=== DEBUG HANDLE_CHILD_PROCESS for command: %s ===\n", cmd->args[0]->value);
	printf("Input fd: %d, Has next command: %d\n", input_fd, cmd->next != NULL);
	printf("Redirection count: %d\n", cmd->redir_count);
