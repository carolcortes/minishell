/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   main.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: cgross-s <cgross-s@student.42porto.com>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/27 17:05:01 by cade-oli          #+#    #+#             */
/*   Updated: 2025/11/24 23:32:40 by cgross-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/minishell.h"

static void	execute_with_redirections(t_command *cmd, t_shell *shell,
	t_token *tokens)
{
	pid_t	pid;

	pid = fork();
	if (pid == 0)
	{
		setup_child_signals();
		exit(handle_child_process_single(cmd, shell, tokens));
	}
	else if (pid > 0)
		handle_parent_process(pid, shell);
	else
	{
		perror("minishell: fork");
		shell->last_status = 1;
	}
}

static void	process_single_command(t_command *cmd, t_shell *shell,
	t_token *tokens)
{
	//////////
	if (cmd->argc == 0)
	{
		if (cmd->redir_count > 0)
		{
			/*if (!apply_redirections(cmd, shell, tokens))
				shell->last_status = 1;
			else
				shell->last_status = 0;*/
			apply_redirections(cmd, shell, tokens);
		}
		return ;
	}
	/////////////
	if (cmd->redir_count > 0)
		execute_with_redirections(cmd, shell, tokens);
	else
	{
		if (is_builtin(cmd->args))
			shell->last_status = exec_builtin(cmd->args, shell->envp);
		else
			shell->last_status = execute_external(cmd->args, shell);
	}
}

/*static void	process_input_line(char *line, t_shell *shell)
{
	t_token		*tokens;
	t_command	*pipeline;

	tokens = shell_split_line_quotes(line);
	free(line);
	if (!tokens)
		return ;
	expand_tokens(tokens, shell);
	
	pipeline = parse_pipeline(tokens, shell);
	///////////
	if (!pipeline)
	{
		shell->last_status = 2;
		free_tokens(tokens);
		return ;
	}
	
	pipeline = parse_pipeline(tokens, shell);

	if (pipeline == (t_command *)-1)            // ERRO REAL
	{
		shell->last_status = 2;
		free_tokens(tokens);
		return;
	}

	if (!pipeline)                              // PIPELINE VÁLIDO mas VAZIO (ex: <<EOF)
	{
		// NÃO altera $?   ← comportamento idêntico ao bash
		free_tokens(tokens);
		return;
	}

	if (pipeline->next)
		execute_pipeline(pipeline, shell, tokens);
	else
		process_single_command(pipeline, shell, tokens);
	free_pipeline(pipeline);
	free_tokens(tokens);
}*/

static void	process_input_line(char *line, t_shell *shell)
{
	t_token		*tokens;
	t_command	*pipeline;

	tokens = shell_split_line_quotes(line);
	free(line);
	if (!tokens)
		return ;

	expand_tokens(tokens, shell);
	pipeline = parse_pipeline(tokens, shell);

	/* ERRO REAL de sintaxe */
	if (pipeline == (t_command *)-1)
	{
		shell->last_status = 2;   // bash usa 2 para erros de sintaxe
		free_tokens(tokens);
		return;
	}

	/* PIPELINE VÁLIDO MAS VAZIO (ex: <<EOF sozinho) */
	if (!pipeline)
	{
		// ⚠️ NÃO modificar shell->last_status
		free_tokens(tokens);
		return;
	}

	/* PIPELINE NORMAL */
	if (pipeline->next)
		execute_pipeline(pipeline, shell, tokens);
	else
		process_single_command(pipeline, shell, tokens);

	free_pipeline(pipeline);
	free_tokens(tokens);
}

static void	main_loop(t_shell *shell)
{
	char	*line;

	while (42)
	{
		setup_signals();
		line = shell_read_line();
		if (g_signal == SIGINT)
		{
			shell->last_status = 130;
			g_signal = 0;
			if (line && !*line)
			{
				free(line);
				continue ;
			}
		}
		if (!line)
			break ;
		process_input_line(line, shell);
	}
}

int	main(int argc, char **argv, char **envp)
{
	t_shell	shell;

	shell.envp = dup_env(envp);
	shell.last_status = 0;
	if (argc >= 3 && ft_strcmp(argv[1], "-c") == 0)
	{
		setup_signals();
		process_input_line(ft_strdup(argv[2]), &shell);
		free_strings(shell.envp);
		return (shell.last_status);
	}
	setup_signals();
	main_loop(&shell);
	free_strings(shell.envp);
	rl_clear_history();
	return (EXIT_SUCCESS);
}
